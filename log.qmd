...
**Next session**: ~~Run Cell 2 (delete + recreate collection), Cell 3 (batched add with embeddings). Verify final count is 31,741. Then resume the original plan: graph expansion Cypher, Gemma 4 E2B triplet filter, hybrid context assembly, side-by-side comparison on the two test queries.~~ DONE — see below.

## Evening session — Chroma rebuild + architectural pivot

**Chroma re-embedding**: Ran the delete+recreate+batch-add cells. First run crashed after 56 minutes with `TypeError: failed to extract enum MetadataValue` — several JSON source files have `None` values for title, authors, or year, and ChromaDB's Rust backend cannot store `None` in metadata. Fixed by adding fallback defaults (`paper.get("title") or "Untitled"`, etc.). Surprising number of papers had null metadata. Re-running with the fix.

**Architectural pivot — triplets, not chunk merging**: Originally planned dual-entry retrieval (vector chunks + graph chunks merged into one pool, then reranked). After working through the implementation, realized that the graph's value isn't finding *more of the same chunks* — it's contributing *structured domain knowledge* that text retrieval fundamentally can't capture. 

New approach: `expand_from_chunks()` takes vector-retrieved chunk IDs, traverses Chunk→MENTIONS→Entity→domain relationships in Neo4j, and returns (subject, predicate, object, confidence) triplets. These get injected as a separate "Related knowledge graph facts" section in the generation prompt, alongside the existing chunk context. No reranking needed for triplets — they're already filtered by confidence ≥ 0.7.

This is stronger for the thesis narrative: the graph and vector stores contribute *different types* of context (structured facts vs unstructured passages), rather than competing to find the same text.

**Model swap completed**: Reformulation now uses DeepSeek API (deepseek-chat) via `USE_LOCAL_REFORMULATION = False` toggle in config. Should significantly reduce query latency vs the old local gemma3:1b calls. Gemma4:e2b available locally as fallback.

**Next session**: Wait for Chroma re-embedding to finish (~40 min). Verify final count is 31,741. Then run the end-to-end test: vector retrieve → graph expand → build enriched context → generate baseline vs hybrid answers for both test queries. Compare outputs qualitatively.